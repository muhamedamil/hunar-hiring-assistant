# Hunar Hiring Assistant — Modules 0 + 1 + 2 + 3 + 4 + 5 + 6 + 7
...
- **Module 6 — Hunar voice submission:** immutable call-submission intent and provider-acceptance certainty.
- **Module 7 — Call results:** signed terminal evidence, human-gated screening answers, and safe GET-only recovery.
...
not call completion. UNKNOWN is never normally retried. Module 7 owns lifecycle/results/recovery and
never rewrites this submission state.
...
Hunar automatic redial is always disabled. Configure the optional summary callback to Module 7's
signed HTTPS receiver for newly created calls.
...
## Module 7 — Call events, screening results and recovery (0.8.0)

Module 7 receives signed `call_summary` webhooks, verifies exact raw bytes before JSON parsing,
and persists one terminal result per Module 6 execution. Only a COMPLETED call explicitly answered
by a HUMAN can produce normalized answers, and each numbered result maps to the exact immutable
...
hiring decision. See [Module 7 validation](doc/MODULE_7_VALIDATION.md).
